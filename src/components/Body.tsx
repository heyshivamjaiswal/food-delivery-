...
import Shimmar from "./Shimmar"; // Import loader
  ...
  function handleSearch() {
    const filtered = allRestaurants.filter((res) =>
      res.info.cuisines?.some((cusine) =>
        cusine.toLowerCase().includes(searchText.toLowerCase())
      )
    );
    ...
    <div className="bg-gray-100 container mx-auto p-5">
      ...
      <div className="flex flex-col sm:flex-row items-center gap-3 mb-6 justify-center z-10 relative">
        ...
        <button
          ...
          className="w-full sm:w-auto bg-gray-800 text-white px-4 py-2 rounded-xl shadow hover:bg-gray-700 transition-all duration-300"
        >
          Search
        </button>
        <button
      ...
      <div className="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {filteredRestaurants.map((restaurant) => (
          <Link
            ...
            className="block rounded-2xl bg-white p-1 shadow-md transition-all duration-300 transform hover:shadow-xl hover:-translate-y-1 hover:scale-[1.02] active:scale-95 active:translate-y-1"
          >
